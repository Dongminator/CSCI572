# this script use multi-threads to download images from the
# url list file "img_urls", store files in separate folders
# 1000 files per folder
# output the count of different types of images when finished

# usage: python3 multi-downloadurl-separate.py
# put "img_ruls" and this script in the same folder
        ...
        for idx,suf in enumerate(suffix):
            if suf+"\n" == token[-1]:
                ...
                if count == 1000:
                    ...
                    subdir += 1
                threadLock.release()
                with open("images/"+s_sub+"/"+file_name, 'wb') as out_file:
