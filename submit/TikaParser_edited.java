...
public class TikaParser implements org.apache.nutch.parse.Parser {
  ...
  public ParseResult getParse(Content content) {
    ...
    // screening out the small images
    boolean isImg = false;
    int imgSize = 1;
    ...
    for (String tikaMDName : TikaMDNames) {
      ...
      if (tikaMDName.equalsIgnoreCase("Image Height")){
    	  isImg = true;
    	  imgSize *= Integer.parseInt(values[0].split(" ",2)[0]);
      }
      else if (tikaMDName.equalsIgnoreCase("Image Width")){
    	  isImg = true;
    	  imgSize *= Integer.parseInt(values[0].split(" ",2)[0]);
      }
      for (String v : values)
    ...
    if(isImg&&imgSize<10000){
    	String message = "This image file is too small. Ignore.";
        LOG.error(message);
        return new ParseStatus(ParseStatus.FAILED, message).getEmptyParseResult(
            content.getUrl(), getConf());
    }
