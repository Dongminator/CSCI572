  ...
  private void readPlainContent(URL url) throws IOException {
    ...
    for (InteractiveSeleniumHandler handler : this.handlers) {
        if (! handler.shouldProcessURL(url.toString())) {
			// If we decide that the URL will not be processed through Selenium, it should use protocol-http
        	// We simply download the HTML content here.
        ...
        System.out.println("=== INFO: driver returned. URL: " + driver.getCurrentUrl());
